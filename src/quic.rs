//! QUIC transport for SpoofTunnel using quiche.
//!
//! This module transports SpoofPacket payloads over a single QUIC
...
struct Inner {
    ...
    recv_tx: mpsc::Sender<SpoofPacket>,
    io_capacity: usize,
}
...
fn drain_frames(buf: &mut BytesMut, tx: &mpsc::Sender<SpoofPacket>) -> Result<()> {
    loop {
        if buf.len() < 4 {
            return Ok(());
        }

...
pub fn apply_auto_tune(cfg: &mut Config) -> Option<TuningSummary> {
    ...
    let runtime_worker_threads = tune_worker_threads(cfg.runtime_worker_threads, cfg.perf_mode, &profile);
...
fn tune_channels(mode: PerfMode, profile: &SystemProfile) -> (usize, usize, usize) {
    ...
    let nic_boost = if nic >= 10000 { 4 } else if nic >= 5000 { 3 } else if nic >= 2000 { 2 } else if nic >= 1000 { 1 } else { 0 };

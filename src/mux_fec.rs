...
//!
//! This layer batches multiple SpoofPacket payloads into a single wire frame
...
) -> Result<Vec<SpoofPacket>> {
    let mut out = Vec::new();

    match frame {
        WireFrame::Data(df) => {
            for p in &df.packets {
                out.push(SpoofPacket::decode(p.clone())?);
            }
...
impl MuxFecSender {
    ...
    pub async fn send(&self, pkt: SpoofPacket) -> Result<()> {
        self.tx
            .send(pkt.encode())
            .await
    ...
    async fn send_frame(&mut self, packets: Vec<Bytes>) -> Result<()> {
        ...
        if self.cfg.enable_fec {
            ...
            if self.group_frames.len() == self.cfg.fec_group_size as usize {
                let parity =
                    encode_parity_frame(self.group_id, self.cfg.fec_group_size, &self.group_frames)?;

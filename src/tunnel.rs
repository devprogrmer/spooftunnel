...
use crate::packet::{SpoofPacket, PacketKind};
...
impl Tunnel {
    ...
    fn apply_syn_ack(&mut self, _syn_ack: &SpoofPacket) -> bool {
    ...
    fn make_data_packet(&mut self, payload: Bytes) -> SpoofPacket {
        let seq = self.send_next;
        self.send_next = self.send_next.wrapping_add(1);
        SpoofPacket::new_data(self.id, seq, payload)
    }
}
...
impl TunnelManager {
    ...
    pub async fn open_tunnel(&self) -> Result<(u32, mpsc::Receiver<Bytes>, mpsc::Sender<Bytes>)> {
        ...
        let syn = SpoofPacket::new_syn(id, syn_seq);
    ...
    pub async fn accept_syn(
        ...
        let syn_ack = SpoofPacket::new_syn_ack(id, our_seq);
    ...
    pub async fn close_tunnel(&self, id: u32) {
        ...
        let fin = SpoofPacket::new_fin(id);
    ...
    pub async fn handle_incoming(
        ...
        src_ip: Ipv4Addr,
        pkt:    SpoofPacket,
    ) -> Result<Option<(SpoofPacket, Ipv4Addr)>> {
        ...
        match pkt.kind {
            ...
            PacketKind::SynAck => {
                if t.apply_syn_ack(&pkt) {
                    ...
                    log::debug!("tunnel {} state={:?}", pkt.tunnel_id, TunnelState::Established);
        ...
        let heartbeats: Vec<_> = {
            ...
            for (id, tunnel) in tunnels {
                    ...
                    && now.duration_since(t.last_heartbeat_sent) >= HEARTBEAT_MIN_INTERVAL
                {
                    ...
                    hbs.push(SpoofPacket::new_heartbeat(t.id, rand::random()));
                }
            }
    ...
    async fn tx_control(&self, pkt: SpoofPacket) -> Result<()> { self.tx_packet(pkt).await }

    async fn tx_packet(&self, pkt: SpoofPacket) -> Result<()> {
        log::trace!(
        ...
        match &self.0.sender {
            PacketSender::Raw { sender, addr, mux_fec } => {
                ...
                let out = match self.0.cfg.uplink_protocol {
                    ...
                    TunnelProtocol::Tcp => {
                        ...
                        OutPacket::Tcp {
                            ...
                            seq,
                            ack,
                            flags:    (pnet_packet::tcp::TcpFlags::PSH | pnet_packet::tcp::TcpFlags::ACK) as u8,
                            payload:  enc,
                        }
                    }
                    TunnelProtocol::Quic => {
...
mod tests {
    ...
    fn server_first_data_seq_is_syn_ack_seq_plus_one() {
        ...
        let syn_ack = SpoofPacket::new_syn_ack(TID, OUR_SYN_ACK_SEQ);
